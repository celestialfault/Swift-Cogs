...
class MemberModule(Module):
    ...
    async def update(self, before: discord.Member, after: discord.Member):
        embed = LogEntry(self, colour=discord.Color.blurple(), description=i18n("Member: {}").format(after.mention))
        ...
        await embed.add_if_changed(name=i18n("Username"), before=before.name, after=after.name,
                                   config_opt=('update', 'name'))

        await embed.add_if_changed(name=i18n("Discriminator"), before=before.discriminator, after=after.discriminator,
                                   config_opt=('update', 'discriminator'))

        await embed.add_if_changed(name=i18n("Nickname"), before=before.nick, after=after.nick,
                                   config_opt=('update', 'nickname'), converter=lambda x: x or inline(i18n("None")))

        await embed.add_if_changed(name=i18n("Roles"), diff=True, config_opt=('update', 'roles'),
