...
class Giveaways(GiveawayBase):
    ...
    async def _giveaway_new(self, ctx: RedContext, *, description: str):
        ...
        The giveaway description can be up to 1500 characters long, any text beyond that limit is trimmed off."""
        ...
        if __len >= 20:
            await ctx.send(warning("Cannot create giveaway as there's already 20 ongoing giveaways in this channel"))
            return
        ...
        embed.set_footer(text="Giveaway started by {0!s}".format(ctx.author))
    ...
    async def _giveaway_list(self, ctx: RedContext, page: int = 1, list_all: bool = False):
        """Lists all currently ongoing giveaways in this channel or the entire guild"""
        ...
        if list_all:  # Guild-wide list
            ...
            __channels = {x: channels[x] for x in channels if len(channels[x])}
            channels = __channels

        else:  # Current channel list
            channels[ctx.channel.id] = list(
                await self.get_giveaways(ctx.guild, ctx.channel, only_ongoing=True))[skip:skip + per_page]
            if not channels[ctx.channel.id]:
                # This results in the no ongoing giveaways message, but, eh - whatever.
        ...
        if not embed.description:
            embed.description = "There's no ongoing giveaways"

        await ctx.send(embed=embed)

    @_giveaway.command(name="info", aliases=["show"])
    async def _giveaway_info(self, ctx: RedContext, giveaway: int):
        """Get info on a specified giveaway"""
        if giveaway < 1:
            await ctx.send(warning("The given giveaway ID must be a non-zero number"))
            return
        ...
        embed.set_author(name="Giveaway #{}".format(index + 1), icon_url=ctx.author.avatar_url)
    ...
    async def _giveaway_end(self, ctx: RedContext, giveaway: int, choose_winner: bool = True):
        ...
        try:
            ...
            if await self.bot.is_owner(ctx.author):
                ...
                await ctx.send(error("The message for that giveaway has been deleted, and as such only "
                                     "moderators can end this giveaway."))
                return
    ...
    async def _giveaway_pin(self, ctx: RedContext):
        ...
        This can be useful if anyone can create giveaways in your guild"""
        async with self.guild_config(ctx.guild)() as settings:
            status = settings.get("pin_messages", False)
            status = not status
            settings["pin_messages"] = status
            await ctx.send(info("Giveaway messages will {}".format("now be pinned" if status
                                                                   else "no longer be pinned")))
    ...
    async def _giveaway_modonly(self, ctx: RedContext):
        ...
        async with self.guild_config(ctx.guild)() as settings:
            status = settings.get("mod_only", True)
            status = not status
            settings["mod_only"] = status
            await ctx.send(info("Giveaways can now be created by {}".format("only moderators" if status
                                                                            else "anyone")))
