...
class RNDActivity:
        ...
        for item in orig_statuses:
            try:
                ...
                if isinstance(item, str):
                    ...
                    status = item["game"]
                statuses.append("{} \N{EM DASH} {!r}  # {}".format(
                    orig_statuses.index(item) + 1, status,
    ...
    def format_status(self, status: Union[str, dict], shard: int = 0,
                      return_formatted=True) -> Union[str, Tuple[str, int]]:
        game_type = 0
    ...
    async def update_status(self, statuses: List[str]):
        ...
        for shard in self.bot.shards.keys():
            try:
                ...
                log.warning("Encountered invalid placeholder {!s} while attempting to parse status "
