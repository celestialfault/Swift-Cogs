...
class Quotes:
    ...
    async def quote(self, ctx: RedContext, quote: int):
        ...
        if quote is None:
            await ctx.send(warning("That quote doesn't exist"))
            return
    ...
    async def _quote_add(self, ctx: RedContext, *, message: str):
        ...
        await ctx.send(tick("Quote added"), embed=quote.embed)
    ...
    async def _quote_message(self, ctx: RedContext, message: int):
        ...
        try:
            ...
            await ctx.send(warning("I couldn't find that message. (is it in a different channel?)"))
        except discord.Forbidden:
            await ctx.send(warning("I'm not allowed to retrieve that message"))
        else:
    ...
    async def _quote_edit(self, ctx: RedContext, quote: int):
        ...
        if not await quote.can_modify(ctx.author):
            await ctx.send(warning("You aren't authorized to modify that quote"))
            return

        embed = discord.Embed(title="Edit Quote",
                              description="What action(s) would you like to take?\n\n"
        ...
        while True:
            async with menu as result:
                if result.timed_out or result == "cancel":
                    ...
                    msg = await prompt(ctx, content="Who would you like to attribute this quote to?", timeout=30.0,
                    ...
                    try:
                        ...
                        await ctx.send(warning(f"Failed to convert `{msg.content}` into a Member - try again?"),
                                       delete_after=30.0)
                        continue
                    else:
                        quote.edited = True
                        ...
                        await quote.save()
                        await ctx.send(tick(f"Attributed quote to **{member!s}**."), delete_after=30.0)

                elif result == "edit_content":
                    msg = await prompt(ctx, content="Please enter the new content for the quote", timeout=120.0,
                                       delete_messages=True)
                    if not msg:
                        continue
                    ...
                    await quote.save()
                    await ctx.send(tick("Modified quote contents successfully."), delete_after=30.0)

                elif result == "delete":
                    if await confirm(ctx, message="Are you sure you want to delete this quote?\n\n"
                                                  "**This action cannot be undone!**",
                                     colour=discord.Colour.red()):
                        await quote.delete()
                        await ctx.send(tick("Quote deleted."), delete_after=30.0)
                        break
    ...
    async def _quote_attribute(self, ctx: RedContext, quote: int, *, author: discord.Member):
        ...
        if quote is None:
            ...
            return
        if not await quote.can_modify(ctx.author):
            await ctx.send(warning("You aren't allowed to modify that quote"))
            return

        quote.edited = True
        quote.message_author = author
        await quote.save()
        await ctx.send(tick(f"Attributed quote #{quote} to **{author!s}**."))
    ...
    async def _quote_remove(self, ctx: RedContext, quote: int):
        ...
        if not quote:
            ...
            return
        if not await quote.can_modify(ctx.author):
            await ctx.send(warning("You aren't authorized to remove that quote"))
            return

        await quote.delete()
        await ctx.send(tick("Quote removed."))
